...
void main( void ) {
  ...
  float loop2 = float(time) / 300.0;
	...
	float shape = polygon(baseCoord, .7, .2, 6);
	vec3 res = vec3(shape);

	vec3 rightGreen = vec3(0.298,0.349,0.118) + 0.3;
	vec3 darkGreen = vec3(0.173,0.251,0.004) - 0.1;

	col = mix(darkGreen, rightGreen, res);
